class CinemaHall:
    ...
    def __init__(self, rows: int, seats: int):
        if rows > self.MAX_NUMBER_OF_ROWS or seats > self.MAX_NUMBER_OF_SEATS:
            raise ValueError(
                f"Maksymalna liczba rzędów wynosi {self.MAX_NUMBER_OF_ROWS}"
                    ...
                    if 0 < place_seat < len(self.__seats[i][1]) + 1:
                        if type_operation == "r":
                            if self.__seats[i][1][place_seat - 1] == "*":
                                ...
                                print(
                                    f"Miejsce {place_seat} w rzędzie {self.__seats[i][0]} zostało zarezerwowane"
                                )
                            else:
                                print(
                            ...
                            if self.__seats[i][1][place_seat - 1] == "X":
                                ...
                                print(
                                    f"Miejsce {place_seat} w rzędzie {self.__seats[i][0]} zostało anulowane"
                                )
                            else:
                                print(
                            ...
                            raise ValueError(
                                "Podano błędy parametr w obsłudze miejsca!"
                            )
                    else:
                        print(
                            f"Podano numer miejsca {place_seat} w rzędzie {self.rows[i]}, którego nie ma!"
                        )
                        break
        else:
    ...
    def __check_all_seats_are_reserved_or_empty(self, mode: str) -> bool:
        ...
        if mode not in ("r", "e"):
            raise ValueError(
                "Parametr 'mode' powinien być równy 'r' lub 'e'."
